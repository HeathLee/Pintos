               ...
               thread_func *function, void *aux) 
{
  ...
  //HEATH: 新的线程被创建之后应立刻使当前正在运行的线程
  //让出CPU， 按照优先权重新调度，以便新创建的高优先级程序
  //能被调度
  thread_yield();
  //END
...
thread_set_priority (int new_priority) 
{
  ...
  if (temp->is_donated && temp->priority > new_priority)
    temp->wanted_priority = new_priority;
  else
...
init_thread (struct thread *t, const char *name, int priority)
{
  ...
  if (thread_mlfqs && t != idle_thread)
  {
    t->priority = FP_INT_PART(FP_SUB_MIX(FP_SUB(FP_CONST(PRI_MAX),
        FP_DIV_MIX(t->recent_cpu, 4)), t->nice * 2));
  ...
  for (; i < 64; ++i)
  {
    t->old_priority_array[i] = -1;
    t->donating_to[i] = 0;
  }

  list_init(&t->donating_thread_list);
...
{
  return list_entry(b, struct thread, elem)->priority < 
...
recalculate_recent_cpu(void)
{
  ...
  for (; e != list_end(&all_list); e = list_next(e))
  {
    ...
    {
      t->recent_cpu = 
        FP_ADD_MIX(FP_MULT(FP_DIV(FP_MULT_MIX(load_avg, 2), 
...
{
  int ready_threads = list_size(&ready_list);//get_ready_threads();
  if (running_thread() != idle_thread)
    ready_threads++;
  load_avg = FP_MULT(FP_DIV_MIX(FP_CONST(59), 60), load_avg) + 
...
void
recalculate_priority_for_all(void)
{
  struct thread *t;
  struct list_elem *e = list_begin(&all_list);
  for (; e != list_end(&all_list); e = list_next(e))
